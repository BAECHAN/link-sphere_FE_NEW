...
조용히 낡아 다음 조사가 같은 일을 반복하게 된다.

#### 후보(우선순위순, 아직 미구현)

| 시나리오                                                                                           | 판정 근거                                                                                                                                                                          |
| -------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 댓글 삭제 — confirm → 게시글 상세·목록·댓글목록 3개 캐시 카운트 감소(+답글 있으면 톰스톤)          | 유닛 0건, 낙관적 레이어 없이 순수 invalidate라 가치는 높으나 어서션이 `comment.spec.ts`(작성→카운트)의 역방향이라 신규 가치가 절반쯤 겹침                                          |
| 게시글 수정 — 제출 즉시 목록 복귀(응답 대기 안 함) → "수정 중..." 오버레이 → direct patch 반영     | `useUpdatePost.test.tsx`가 `onSubmit`을 한 번도 호출 안 함. 다만 500ms 지연 게이트 때문에 타이밍 flaky 위험                                                                        |
| 북마크 폴더 삭제 — ⋮ → confirm → 목록에서 사라짐 + URL이 `all`로 복귀                              | `/bookmark` 페이지 첫 e2e. `onBeforeDelete`가 DELETE 전에 URL을 옮기고 그 이동이 `location.key`를 바꿔 Alert가 자동 취소되는 상호작용이 핵심. 훅 유닛은 이미 두꺼움, UI 테스트만 0 |
| 공개/비공개 전환 — confirm → invalidate → 재조회로만 반영(direct patch 없음)                       | 유닛 0건이나 `comment.spec.ts`가 쓴 stateful mock 패턴의 반복이라 새로 배우는 게 적음                                                                                              |
| 검색 필터 cross-layer — 칩 클릭 → URL 파라미터 → localStorage(`hideBots`) 합성 → 실제 API 파라미터 | 로그인 불필요, 가장 싼 스펙. 기존 유닛(파싱 11 + 카운트 표시 3)은 순수 함수와 표시만 덮음                                                                                          |
