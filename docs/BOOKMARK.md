...
체크된 미분류가 no-op인 이유: 미분류는 소속 row가 0개인 파생 상태라 지울 대상
자체가 없다. 폴더 행은 삭제 후 그 폴더로 다시 추가해 되돌릴 수 있지만, 미분류는
되돌릴 폴더가 없어 Undo가 불가능한 파괴적 조작이 된다 — 그래서 no-op으로 막는다
(완전 삭제는 소속 스냅샷으로 되돌릴 수 있는 `북마크 제거` 행에서만). 소속된 모든
폴더에 **동일한 ✓ 아이콘**이 표시된다(다중 선택 UI가 아니라, 탭할 때마다 즉시
반영되는 토글 방식).
