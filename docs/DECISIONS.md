...
**결정 (최초, 실제 적용 시 정정됨 — 아래 "적용 중 정정" 참고)**

- WAF: `SizeRestrictions_BODY` → Count 오버라이드 + "바디 16,384바이트 초과 → Block" 커스텀
  룰 신설. 구성·재적용 절차는 `docs/DEPLOY.md`의 "CloudFront WAF (수동 관리)" 절.
- 앱: BE·FE 모두 UTF-8 12,000바이트(한글 약 4,000자) 상한. BE `CommentService.createComment`/
  `createReply`/`updateComment`, FE `commentContentFormSchema` 공유 스키마.
- FE: `useCreateComment`가 실패 시 입력·이미지를 복원(사용자가 그 사이 새로 입력을
  시작했으면 덮어쓰지 않음).

**적용 중 정정 — WAF 커스텀 크기 제한 룰은 Pro 플랜 전용이었다**

실제 적용 시 `SizeConstraintStatement`(커스텀 크기 제한 룰)가
`WAFFeatureNotIncludedInPricingPlanException`으로 거부됐다 — 이 기능은 CloudFront **Pro
플랜($15/월 정액제) 이상**에서만 지원되는데 이 계정은 Free 플랜이었다. Pro 업그레이드
(월 $15 고정비 발생)와 "업그레이드 없이 Count만 적용"을 사용자에게 직접 물었고, **Count만
적용**을 선택했다 — 결과적으로 **WAF 레이어의 바디 크기 방어선은 없어졌다**(Lambda 자체
한도 6MB까지 통과, 300KB 페이로드로 실측 확인). 댓글 본문의 유일한 크기 방어는 이제 위
앱 레벨 12,000바이트 검증뿐이고, 그 외 엔드포인트(게시글 등)는 크기 기반 방어가 전혀
없다. Pro로 업그레이드하면 되살릴 수 있는 룰 설정은 `docs/DEPLOY.md`에 남겨뒀다.

**상태**

적용 완료(WAF는 콘솔/CLI 수동 적용 — 코드로 추적 불가, 위 문서 참고). 게시글
(`POST`/`PATCH /post`) 본문 길이 제한은 이번 범위 밖으로 남겨뒀다 — Pro 미적용 상태라
WAF의 개선도 없고 앱 상한도 없다(위 정정 참고).
