...
| _이번 preload 반영 후 채울 것_               | 프로덕션 `/post`, 모바일(느린 4G) |             |           |          |             |       |                                                                                   |

데스크톱은 개선됐지만 모바일(느린 4G)은 오히려 나빠졌다 — 이유와 대응이 바로 아래
"폰트 preload 유지보수" 절이다. 위 "코드 변경 전후 비교"는 애초에 왜 개선을 기대했는지의
근거였을 뿐, 실제 모바일 결과와는 어긋났다. 계획 대비 구현 전체 대조는
`docs/plans/2026-09-25-lighthouse-perf.md`와 PR 본문의 "계획 대비 구현"을 본다.
