...
업계 다수 관행(Baymard 가이드라인 #346 — 데스크톱 78%가 검색어 유지)과 어긋나는 소수파
패턴이었다. 이 문서는 "검색어가 지금 어떻게 저장·표시되는가"를 한곳에 정리해, 다음에 이
...
보였지만, 실제로는 Google 결과 페이지의 Clear 버튼과 네이티브 `<input type="search">` 모두
입력만 비운다는 게 확인되어 현행(입력만 비움)을 유지했다. 자세한 경위는
[`docs/DECISIONS.md`](./DECISIONS.md) "2026-09-07" 참고.
