...
### 2026-09-25 (FE)
- 게시글 등록 화면 내 특정 팀을 지칭하는 부적절한 안내 문구 수정 ([#193](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/193))

### 2026-09-24 (FE)
- 백엔드 운영 스펙 스냅샷 갱신을 통한 빌드 환경 동기화
...
### 2026-09-24 (FE)
- E2E 테스트 환경 안정성 강화를 위해 전용 포트 분리
  - E2E 테스트용 webServer를 dev 서버와 독립된 포트로 구성하여 빌드 및 테스트 환경 충돌 방지
...
### 2026-09-21 (FE)
- 브랜드 컬러 적용을 철회하고 디자인 시스템의 무채색 체계로 원복

### 2026-09-21 (FE)
- 브랜드 컬러 시스템 업데이트: 기존 블루 계열에서 틸(Teal) 계열로 색상 팔레트 변경 및 적용

### 2026-09-21 (FE)
- 파비콘 디자인을 브랜드 틸(Teal) 색상으로 업데이트하여 브랜드 아이덴티티 일관성 강화

### 2026-09-21 (FE)
- 디자인 시스템 고도화를 위한 색상, 타이포그래피, 입체감 디자인 토큰 전면 개편

### 2026-09-21 (FE)
- 최근 검색 드롭다운 내 화살표 키 내비게이션 로직 개선 및 스크롤 추적 기능 추가
- 계획 스냅샷 문서화 및 업데이트

### 2026-09-21 (FE)
- 데스크톱 헤더 검색 기능 내 최근 검색어 드롭다운 UI 구현
...
### 2026-09-16 (FE)
- 검색 플레이스홀더 텍스트 일관성 개선 및 불필요한 말줄임표 제거를 통한 UI 가독성 향상

### 2026-09-16 (FE)
- 모바일 헤더 레이아웃 최적화: 좌측 아이콘 위치를 컨테이너 가장자리에 맞추어 시각적 정렬 개선 ([#124](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/124))

### 2026-09-16 (FE)
- 모바일 검색 오버레이 UX 개선 (댓글 작성바 숨김 및 배경 요소 inert 처리)
...
### 2026-09-16 (FE)
- ESLint 커스텀 룰 도입 및 역할 토큰 추가를 통한 코드 품질 개선 및 타입 시스템 강화 ([#116](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/116))
...
### 2026-09-16 (FE)
- 타이포그래피 디자인 시스템 구축 및 스타일 토큰 적용 ([#108](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/108))

### 2026-09-15 (FE)
- 상세 페이지 내 '돌아가기' 버튼의 레이아웃을 gap 속성 기반으로 재구현하여 UI 간격 정밀도 개선

### 2026-09-15 (FE)
- 게시글 상세 페이지 내 '뒤로 가기' 버튼과 카드 컴포넌트 간의 레이아웃 간격 최적화 ([#106](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/106))

### 2026-09-15 (FE)
- /post 목록 내 요소 간 세로 간격 축소 및 레이아웃 최적화

### 2026-09-14 (FE)
- 게시글 상세 페이지 UI 개선: 모바일 환경에서 돌아가기 버튼을 제거하고, 데스크톱 환경의 sticky 속성을 해제하여 레이아웃 최적화

### 2026-09-14 (FE)
- 상세 페이지 내 돌아가기 버튼 sticky 레이아웃 적용 및 유입 경로에 따른 라벨 표시 로직 최적화 ([#100](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/100))

### 2026-09-14 (FE)
- 로그아웃 시 불필요한 데이터 재요청 방지를 통한 성능 최적화

### 2026-09-14 (FE)
- 댓글 영역 스크롤 위치 보정을 위한 navbar 여백 24px 적용 ([#95](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/95))

### 2026-09-14 (FE)
- 재로그인 시 이전 로그아웃 과정에서 발생한 에러 캐시가 남아있던 문제를 해결하여 인증 흐름 안정화
...
### 2026-09-14 (BE)
- 내 댓글 목록 조회 API 기능 구현
- 댓글 히스토리 조회 기능을 위한 기반 로직 추가 및 명세 정리
...
### 2026-09-14 (FE)
- E2E 테스트 티어 3 시나리오 5건 추가 및 모바일 프로젝트 배선 작업 완료

### 2026-09-14 (BE)
- PostResponse 응답 객체 구조 최적화 및 중복 필드 제거를 통한 데이터 효율성 개선

### 2026-09-14 (FE)
- UI/UX 일관성 개선 및 인증 페이지 레이아웃 최적화
  - 아이콘, 제목, 여백, 필터칩 스타일 및 인증 화면 시각 요소 수정 반영 ([#85](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/85))

### 2026-09-14 (FE)
- Post 응답 데이터 구조 최적화 및 타입 중복 제거를 통한 코드 유지보수성 향상

### 2026-09-13 (FE)
- 디자인 시스템 체계화 및 프로젝트 품질 관리 강화
  - 디자인 토큰(z-index, 색상) 도입을 통한 스타일 일관성 확보
  - ESLint 규칙 강제화로 코드 품질 표준화
  - 컴포넌트 구조 리팩토링을 통한 유지보수성 향상
...
### 2026-09-11 (FE)
- 비로그인 상태에서 북마크 시도 후 로그인 시 해당 동작이 자동으로 재개되도록 워크플로우 개선
- 공유 아이콘 렌더링 오류 수정 및 CHANGELOG 데이터 무결성 복구

### 2026-09-11 (FE)
- 마우스 고정(pointer lock) 상태에서 커서 스타일이 반복적으로 변경되던 이슈 수정 및 UI 깜빡임 현상 해결
...
### 2026-09-10 (FE)
- 인증 에러 처리 개선 및 문서화 오류 수정
  - 로그인 실패 시 서버 원문 에러 메시지 노출 현상 수정
  - 프로젝트 내 잔여 문서 오류 4건 현행화
...
### 2026-09-09 (BE)
- YouTube AI 요약 기능 복구 및 장애 대응 기록
- Data API v3를 활용한 YouTube 본문 확보 폴백 로직 구현
- 백필(backfill) 검증 결과 및 dry-run 운영 가이드 현행화
...
- CI 파이프라인에 문서-코드 참조 정합성 검사 단계 추가
- 아키텍처 패턴 서술을 FE-ARCHITECTURE 문서로 단일화 및 최신 FSD 구조 반영
- ESLint 규칙 참조 방식 개선 및 낡은 코드 구조/경로 서술 정정

### 2026-09-07 (BE)
- YouTube 자막 크롤링 최적화 및 AI 요약 기능 개선
- YouTube 영상 설명의 인라인 JSON에서 본문 데이터를 추출하도록 기능 구현
- AI 요약 백필(Backfill) 기능에 도메인 지정 재분석 및 분할 처리 로직 도입
- 크롤링 데이터 무결성 검증 강화 (유효하지 않은 본문 데이터 null 처리 및 폴백 경로 확보)
...
### 2026-09-06 (FE)
- 게시글 상세 페이지 폰트 Preload 설정 최적화 및 댓글 작성 UI 레이아웃 이슈 수정

### 2026-09-06 (FE)
- 필터 초기화 버튼의 활성화 로직 개선을 통한 사용자 경험 최적화

### 2026-09-06 (FE)
- PWA 매니페스트 아이콘 경로 수정 및 댓글 썸네일 폴백 로직 구현
- Og-image 썸네일 생성 워크트리 기능 통합 및 예외 처리 강화

### 2026-09-06 (BE)
- og:image 메타 태그의 상대 경로를 절대 URL로 변환하여 소셜 공유 시 이미지 표시 오류 해결

### 2026-09-06 (FE)
- 댓글 전송 시 개행 및 이미지 데이터로 인한 페이로드 과다 문제를 해결하여 WAF 차단 이슈 재발 방지 및 전송 안정성 확보

### 2026-09-06 (FE)
- 댓글 본문 길이 제한 기능 구현 및 입력 실패 시 텍스트 복원 로직 추가
- WAF(Web Application Firewall) 정책 최적화 및 상한 재조정
- 댓글 입력 초과 안내 문구 노출 버그 수정 및 관련 문서 현행화

### 2026-09-06 (BE)
- WAF 설정 원복 및 댓글 본문 길이 제한 정책 최적화
- 댓글 본문 길이를 UTF-8 기준 12,000바이트로 재조정하여 WAF 8KB 제한 대응 및 시스템 안정성 확보

### 2026-09-06 (FE)
- 북마크 위젯 4종에 Headless Hook 패턴 적용 및 Container/Presentational 구조 분리로 로직과 UI 분리 최적화

### 2026-09-06 (FE)
- 봇 글 숨기기 기능의 로직을 초기화 행으로 재배치하여 안정성을 높이고, 게시글 텍스트 크기를 통일하여 UI 일관성 개선
