 ...
 * 하나 등록해(id 06ec0958-a33c-4c3f-81f6-0481badbbeb7, 공개) 해소했다. 원래
 * https://www.inflearn.com/였으나, BE 스크래퍼가 이 URL을 일본어로 긁어온 문제가
 * 반복 재현돼(원인 미확정 — 추정: BE Lambda가 ap-northeast-1에서 나가는 요청을
 * inflearn의 CDN이 다른 지역으로 취급) 같은 게시글 ID를 유지한 채 URL만
 * https://tech.kakao.com/(한국어로 안정적으로 스크랩됨, 2026-09-26 확인)로 바꿨다.
 * 이 게시글은 측정용으로만 쓰고 삭제하지 않는다 — 지우면 이 설정이 다시 깨진다.
 */
module.exports = {
  ci: {
    collect: {
