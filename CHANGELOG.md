...
- **모바일 스와이프 메뉴 전환** — 모바일 화면에서 좌우 드래그로 인접 메뉴(홈 · 링크
  등록 · 북마크)를 전환. 콘텐츠가 손가락을 따라 움직이고, 임계값 미달 시 원위치로
  복귀(스프링백). 거리(80px) 또는 속도 임계값을 넘기면 해당 탭으로 이동. 글쓰기 화면은
  폼 입력 간섭을 줄이도록 임계값을 크게(160px) 설정. 상세 · 편집 페이지와 데스크탑에서는
  비활성. (framer-motion 드래그 활용)
