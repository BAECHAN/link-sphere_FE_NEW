...
- `shared` `/post` 피드 로딩 성능 개선: 첫 화면 썸네일 우선순위, 폰트 dynamic subset 전환, 비로그인 방문자의 Firebase 지연 로딩
  ...
  프로덕션 `/post` Lighthouse 측정(데스크톱 LCP 2.3s, PageSpeed Insights 모바일 LCP 9.4s, 2026-09-25~26)을 계기로 세 가지를 고쳤다. (1) 목록 첫 행(최대 3장) 썸네일에 `loading="eager"`+`fetchPriority="high"`를 줬다 — 실측 결과 프로덕션 LCP 요소가 항상 이 썸네일이었는데 `loading="lazy"`로 처리되고 있었다(`LinkThumbnail`에 `priority` prop 신설, `PostList`가 가상화 첫 행에만 전달). (2) 정적 굵기별 Pretendard subset 9종(총 preload 1.07MB) + 안 쓰는 Inter Google Fonts를 Pretendard 공식 "dynamic subset"(가변 폰트 + unicode-range 92분할, 화면에 쓰인 글자만큼만 파일당 20~40KB씩 받음)으로 교체했다 — 당근마켓·마켓컬리 프로덕션을 직접 실측한 결과 두 회사 모두 이 공식 배포판을 그대로 쓰고 있었다(`docs/DECISIONS.md` 2026-09-26 항목 참고). (3) Firebase(FCM)가 비로그인 방문자에게도 정적으로 번들돼 있던 것을 로그인 상태에서만 동적 import하도록 바꾸고, `vite.config.ts`의 `manualChunks`에 firebase 전용 청크를 분리했다(실측: 분리 전엔 import 방식만 바꿔선 다른 정적 의존성과 같은 `vendor` 청크로 묶여 효과가 없었다) — 같은 이유로 zod도 react-hook-form과 분리해 폼이 없는 페이지가 폼 라이브러리까지 받지 않게 했다. `LoginModal`도 자체 Suspense 경계로 지연 로딩한다. 측정 자동화(Lighthouse CI, `pnpm perf:lh`)도 함께 도입해 PR마다 공개 페이지 성능을 `warn` 단계로 리포트한다(`docs/DECISIONS.md` 2026-09-26 "Lighthouse CI" 항목).
  (`src/shared/ui/atoms/link-thumbnail.tsx`, `src/widgets/post/post-card/ui/PostCard.tsx`, `src/widgets/post/post-list/ui/PostList.tsx`, `index.html`, `src/app/globals.css`, `src/shared/lib/firebase/fcm.ts`, `src/shared/lib/firebase/useFcmForegroundMessage.ts`, `src/app/routes/layouts/RootLayout.tsx`, `vite.config.ts`, `lighthouserc.cjs`(신규), `lighthouserc.auth.cjs`(신규), `.github/workflows/ci.yml`, `docs/PERFORMANCE.md`(신규), `docs/plans/2026-09-25-lighthouse-perf.md`(신규), [PR #195](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/195))
...
- `post` file:// 등 크롤링 불가능한 URL을 등록하면 원인 없이 실패하던 문제
  ...
  (`src/entities/post/model/post.schema.ts`, `src/shared/config/texts.ts`, `docs/plans/2026-09-26-post-url-scheme-validation.md`(신규))
...
- `infra` doc-drift 경량 감사가 5개 병합 누적 대신 마지막 push 1건만 검사하던 문제 수정
  ...
  (`scripts/check-doc-drift.js`, `.claude/CLAUDE.md`, `docs/plans/2026-09-25-doc-drift-audit-range.md`(신규), [PR #194](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/194))
...
- `post` 링크 등록 화면 안내 문구가 이 서비스를 팀 전용 도구처럼 표현하던 문제
  ...
  (`src/shared/config/texts.ts`, [PR #193](https://github.com/BAECHAN/link-sphere_FE_NEW/pull/193))
