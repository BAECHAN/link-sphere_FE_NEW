...
- 폰트가 화면이 다 로딩된 뒤에야 적용되는 것처럼 보였던 문제. 원인은 본문에 쓰는
  Pretendard를 단일 가변 폰트 파일(2.0MB)로 서빙하고 있어서 `font-display: swap`이
  걸려 있어도 이 큰 파일이 완전히 받아질 때까지 스왑이 지연되고, 다른 JS/이미지/API
  요청과 대역폭을 다퉜기 때문. 실제 앱에서 쓰는 굵기는 400/500/600/700뿐이라 가변
  폰트의 연속 보간 기능이 쓰이지 않고 있어, 굵기별 서브셋 정적 폰트(각 ~260KB)로
  되돌리고 preload도 Regular/Medium/Bold로 맞춤. (`globals.css`, `index.html`)
