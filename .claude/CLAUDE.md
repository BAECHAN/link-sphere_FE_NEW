...
- **독립 기능 문서** (`docs/<기능명>.md`) — 자기 완결적인 동작 스펙을 가진 기능.
  ...
  `BOOKMARK.md`, `MYPAGE.md`, `FCM-PUSH-NOTIFICATION.md`(이미 이 형태로
  "삽질 기록" 절을 포함하고 있음), `UNSAVED-CHANGES-GUARD.md`.
...
- **설계 결정 기록** (`DECISIONS.md`, ADR 경량판) — **되돌리기 어렵고, 실제로
...
보내지 않고 기능 문서 안에 남긴다는 점만 BE와 다르다):

1. **쉬운 설명** — 핵심 개념을 이 도메인을 모르는 독자도 이해할 수 있는 비유로
2. **사용한 도구·기술** — bullet point, "기능 자체" / "구현·검증 과정에서 쓴 도구" 구분
3. **왜 만들었나** (문제)
4. **구조** — 다이어그램·핵심 설계 결정과 그 근거
5. **운영 파라미터** — 있다면(주기·건수·타임아웃 등) 표로, 각 값의 실제 위치를
   `파일:줄`까지 명시
6. **검증 결과** — 실제로 확인한 수치·화면
7. **시행착오** — 겪은 버그와 디버깅 과정. **뒤쪽에 배치** — 결과를 다 본 사람을
