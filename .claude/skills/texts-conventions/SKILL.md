...
1. **가시성** — 액션 직후 현재 화면에서 결과가 바로 보이는가? (목록에서 사라짐·이름
   ...
   `postUpdated`/`accountUpdated`를 이 이유로 "필요" 쪽으로 재분류). "토글"도
   **낙관적 업데이트가 있을 때만** 이 전제가 성립한다 — `onMutate` 없이
   `invalidateQueries`만 하는 토글은 아이콘 전환이 서버 왕복 뒤에야 일어나 눌러도
   즉시 아무 일도 안 일어난 것처럼 보인다(2026-09-21 확정: `postVisibilityUpdated`
   재분류 사유 참고).
...
> **2026-09-21 `postVisibilityUpdated` 재분류.** 2026-08-04에 "아이콘 상태 전환으로 이미
> 보임"을 근거로 이 키를 "불필요"로 분류하고 제거했었다. 그 근거가 실제 코드와 맞지
> 않았다 — 이 토글이 시각적으로 나타내는 곳은 `PostCard.tsx`의 자물쇠 버튼 하나뿐이고
> `isOwner && post.isPrivate` 조건이라 공개 전환 시엔 아이콘이 아예 사라지며, 낙관적
> 업데이트도 없어 재조회가 끝나야 반영된다(주 진입점인 드롭다운은 그 전에 닫힌다). 방향별
> 문구(`postSetToPrivate`/`postSetToPublic`) 2키로 다시 추가했다.
