...
- 요약 줄: `` `스코프` `` + 공백 + 한 줄(72자 이내, 줄바꿈·마침표 없음). 굵게(`**`) 쓰지 않는다.
  (문자 수 기준. 0.1.0~0.14.0대 과거 릴리즈 섹션에 초과 항목이 일부 남아 있으나
  소급 적용하지 않는다 — `[Unreleased]` 신규 항목은 전부 이 규칙을 지킨다.)
...
- 버전 체계 기준점: `0.1.0`부터 시작(정식 릴리즈 전 개발 단계 = `0.x`). 현재 버전은
  ...
  이 값은 릴리즈마다 바뀌므로 여기 고정된 숫자로 적지 않는다. `package.json`의 `version`은
  `0.0.0`으로 고정돼 있고 쓰지 않는다(`release.yml`도 이 필드를 참조하지 않는다)
