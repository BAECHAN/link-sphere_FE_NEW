 ...
 *   찍고, 매 렌더마다 최신 `folderList`에서 그 id들을 다시 찾아 반환한다 — 그래야 스냅샷 이후
 ...
 *   `folderList`가 바뀌어도(재검증·다른 탭에서 저장 등) 구성/순서를 다시 계산하지 않는다 — 원칙4.
 ...
 *   컴포넌트 마운트 시점에 곧바로 계산해서도 안 된다: 로딩 중엔 `folderList`가 빈 배열이라, 그
 *   순간 값을 굳혀버리면 데이터가 도착해도 영원히 빈 채로 고정된다.
 * - `sessionKey`(선택): BookmarkFolderSelectModal처럼 열고 닫을 때마다 새 스냅샷을 찍고
 *   싶은 호출부가 그 open 상태를 넘긴다. 지금은 모달(useBookmarkFolderSelect)이 유일한
 *   호출부다 — 상시 마운트 화면(FolderTree/MobileFolderList)은 세션 경계가 없어 스냅샷이
 *   영영 안 풀렸고(2026-09-21), 그래서 useFolderSections는 BookmarkFolderUtil.pickRecentFolders를
 *   직접 쓴다.
...
export function useRecentBookmarkFolders(
  ...
  useEffect(
    function snapshotRecentFolderIdsOnceSettled() {
      ...
      setRecentFolderIds(
        BookmarkFolderUtil.pickRecentFolders(folderList).map((folder) => folder.id)
      );
    },
    [folderList, isFetching, sessionKey]
  );

  const recentFolderList = recentFolderIds
    .map((id) => folderList.find((folder) => folder.id === id))
    .filter((folder): folder is BookmarkFolder => folder !== undefined);

  return { recentFolderList };
}
